...
test.describe('Ably Integration E2E Tests', () => {
    ...
    test('Complete Ably real-time game flow with two players', async ({ browser }) => {
        console.log('🎮 Starting Ably Integration E2E Test - Complete Game Flow');
        ...
        try {
            ...
            console.log('✅ Both users navigated to app');
            ...
            await user1Page.fill('input[name="userName"]', 'AblyPlayer1');
            ...
            await user2Page.fill('input[name="userName"]', 'AblyPlayer2');
            ...
            await user1Page.fill('input[placeholder*="game name"]', 'Ably Test Game');
            await user1Page.click('button:has-text("Create Game")');

            // Wait for game creation and redirect
            await user1Page.waitForSelector('text=Game Board', { timeout: 10000 });
            ...
            await user2Page.waitForSelector('text=Ably Test Game', { timeout: 10000 });
            ...
            await user2Page.waitForSelector('text=Game Board', { timeout: 10000 });
            ...
            await user1Page.waitForSelector('text=AblyPlayer1\'s turn', { timeout: 10000 });
            console.log('✅ Game started - AblyPlayer1\'s turn');
            ...
            await user2Page.waitForSelector('text=AblyPlayer2\'s turn', { timeout: 10000 });
            ...
            const user1GameEnded = await user1Page.locator('text=AblyPlayer1 wins').isVisible() ||
                await user1Page.locator('text=AblyPlayer2 wins').isVisible() ||
                await user1Page.locator('text=It\'s a draw').isVisible();

            const user2GameEnded = await user2Page.locator('text=AblyPlayer1 wins').isVisible() ||
                await user2Page.locator('text=AblyPlayer2 wins').isVisible() ||
            ...
            await user1Page.fill('input[placeholder*="message"]', 'Great Ably-powered game!');
            await user1Page.click('button:has-text("Send")');
            await user1Page.waitForTimeout(2000);

            // Check if User 2 receives the message in real-time
            const messageReceived = await user2Page.locator('text=Great Ably-powered game!').isVisible();
            expect(messageReceived).toBeTruthy();
            console.log('✅ Chat message sent and received via Ably');

            // User 2 responds
            await user2Page.fill('input[placeholder*="message"]', 'Amazing real-time sync!');
            await user2Page.click('button:has-text("Send")');
            await user2Page.waitForTimeout(2000);

            // Check if User 1 receives the response
            const responseReceived = await user1Page.locator('text=Amazing real-time sync!').isVisible();
            ...
            console.log('\n🎉 Ably Integration E2E Test Completed Successfully!');
            console.log('✅ All real-time features working perfectly with Ably');

        } catch (error) {
            console.error('❌ Ably E2E Test Failed:', error);

            // Take screenshots for debugging
            await user1Page.screenshot({ path: 'ably-user1-error.png' });
            await user2Page.screenshot({ path: 'ably-user2-error.png' });
    ...
    test('Ably connection stability and reconnection', async ({ page }) => {
        console.log('🔗 Testing Ably connection stability and reconnection');
        ...
        await page.fill('input[name="userName"]', 'StabilityTestUser');
        await page.fill('input[name="password"]', 'password123');
        await page.click('button[type="submit"]');

        // Wait for lobby and connection
        await page.waitForSelector('text=Welcome to Tic-Tac-Toe Online', { timeout: 10000 });
        await page.waitForSelector('text=Real-time connected', { timeout: 10000 });

        console.log('✅ User registered and connected to Ably');
        ...
        console.log('🎉 Ably connection stability test completed successfully');
    });

    test('Ably rate limiting and performance', async ({ browser }) => {
        console.log('⚡ Testing Ably rate limiting and performance');
        ...
        try {
            // Create 5 browser contexts to simulate multiple users
            for (let i = 0; i < 5; i++) {
            ...
            const registrationPromises = pages.map((page, index) =>
                page.click('text=Don\'t have an account? Sign up').then(() =>
                    page.fill('input[name="userName"]', `RateTestUser${index + 1}`)
                ).then(() =>
            ...
            await Promise.all(pages.map(page =>
                page.waitForSelector('text=Welcome to Tic-Tac-Toe Online', { timeout: 10000 })
            ));

            console.log('✅ All 5 users registered and connected to Ably');

            // Test simultaneous game creation
            const gameCreationPromises = pages.slice(0, 3).map((page, index) =>
                page.click('text=Create New Game').then(() =>
                    page.fill('input[placeholder*="game name"]', `RateTestGame${index + 1}`)
                ).then(() =>
            ...
            await Promise.all(pages.slice(0, 3).map(page =>
                page.waitForSelector('text=Game Board', { timeout: 10000 })
            ));

            console.log('✅ Multiple games created simultaneously via Ably');
            ...
            await pages[4].waitForLoadState('networkidle');

            // Check that games appear in lobby
            const gamesVisible = await Promise.all([
                pages[3].locator('text=RateTestGame1').isVisible(),
                pages[3].locator('text=RateTestGame2').isVisible(),
                pages[3].locator('text=RateTestGame3').isVisible(),
                pages[4].locator('text=RateTestGame1').isVisible(),
                pages[4].locator('text=RateTestGame2').isVisible(),
                pages[4].locator('text=RateTestGame3').isVisible(),
            ]);

            expect(gamesVisible.every(visible => visible)).toBeTruthy();
            console.log('✅ All games visible to all users via Ably real-time updates');
            ...
            for (let i = 0; i < 10; i++) {
                rapidMessages.push(
                    pages[0].fill('input[placeholder*="message"]', `Rapid message ${i + 1}`).then(() =>
            ...
            const messagesSent = await pages[1].locator('text=Rapid message').count();
            expect(messagesSent).toBeGreaterThan(0);
            console.log(`✅ Rate limiting handled ${messagesSent} rapid messages gracefully`);

            console.log('🎉 Ably rate limiting and performance test completed successfully');

        } catch (error) {
            console.error('❌ Ably rate limiting test failed:', error);
            throw error;
        } finally {
